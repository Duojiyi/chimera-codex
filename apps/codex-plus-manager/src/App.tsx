...
type Route = "overview" | "relay" | "relayEnvironment" | "sessions" | "context" | "enhance" | "dreamSkin" | "zedRemote" | "userScripts" | "maintenance" | "settings";
...
export function App() {
  ...
  useEffect(() => {
    document.title = DISPLAY_MANAGER_NAME;
  }, []);
    ...
    },
    {
      ...
      detail: result
        ? proxyVariables.length
          ? tf("检测到代理环境变量：{0}。请清理后重新启动 Chimera++。", [proxyVariableLabels.join(t("、"))])
  ...
  return (
    <>
      ...
      <Panel className="dream-skin-panel">
        <CardHead title={t("运行状态")} detail={t("配置保存在 Chimera++，实时操作通过本机回环 CDP 执行")} />
        <CardContent>
      ...
      <Panel className="dream-skin-panel">
        <CardHead title={t("图片与主题")} detail={t("自定义图片会被导入 Chimera++ 托管目录；主题字段与目标项目 theme.json 对齐")} />
        <CardContent>
