...
async fn main() -> anyhow::Result<()> {
    ...
    println!("Chimera++ mobile relay listening on ws://{local_addr}");
    println!(
...
<head>
  ...
  <title>Chimera++ Mobile Relay</title>
  <style>
...
<body>
<div class="app">
  <header class="topbar"><strong class="title">Chimera++</strong><span id="status" class="status">待连接</span></header>
  <main class="layout">
...
async function ensureAppServer() {
  ...
  await rpcRaw("initialize", { clientInfo: { name: "Chimera++ Mobile Relay", version: "1.0.0" }, capabilities: { experimentalApi: true } });
}
