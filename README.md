...
<p align="center">
  <img src="docs/images/codex-plus-plus.png" alt="Chimera++ 图标" width="160">
</p>
...
Chimera++ 是面向 ChimeraHub 客户的 Codex App 外部增强启动器与管理工具。它不修改 Codex App 原始安装文件，通过外部 launcher 启动 Codex，并注入所需增强能力。

本发行版默认对接 **ChimeraHub** 中转，去除推广、赞助和社区导流入口。普通用户无需了解代码仓库或更新源。

> 发行准备状态：当前开发快照尚未完成单桌面入口和自动更新强制策略验收，不得作为客户正式发行版交付。下述单桌面与自动更新内容描述的是完成后目标发行行为。

## 快速使用

从 ChimeraHub 客服或客户交付页面获取与你的电脑匹配的安装包：

- Windows：`ChimeraPlusPlus-*-windows-x64-setup.exe`（另有 zip 便携包）
- macOS Intel：`ChimeraPlusPlus-*-macos-x64.dmg`
- macOS Apple Silicon：`ChimeraPlusPlus-*-macos-arm64.dmg`

目标发行版的 Windows 桌面只创建一个 `Chimera++` 入口。桌面 `Chimera++` 入口会打开管理工具，用户可从管理工具启动 Codex；`Chimera++ 管理工具` 也保留在开始菜单，用于配置、诊断和维护。macOS 安装包包含两个 App，日常使用从 `Chimera++.app` 启动。

- `Chimera++`：Windows 日常管理与启动入口；macOS 日常启动入口。
- `Chimera++ 管理工具`：首次填写 Key，以及后续诊断、修复和高级配置入口。
...
1. 退出正在运行的 `Codex++` / `Chimera++` 及相关管理工具进程。
2. 运行新的 `ChimeraPlusPlus-*-windows-x64-setup.exe`。
3. 安装程序会清理旧快捷方式后，只创建 Chimera 入口。
4. 从新的桌面 / 开始菜单入口启动即可。

更新组件匿名读取 Chimera++ 的公开更新清单，不会回落到其他更新源。
...
2. 打开 DMG，将 `Chimera++.app` 与 `Chimera++ 管理工具.app` 拖入 `/Applications`。
...
ChimeraHub Key-first 默认使用 Pure API 路径，实际写入的 `config.toml` 形态为：
...
`custom` 是 Codex 配置中的 provider id，不是产品显示名；Key-first 不使用 mixed 模式的 `experimental_bearer_token`。

## 自动更新

目标发行版的 Chimera++ 会在启动时自动检查并安装更新。普通更新失败或网络不可用时，仍受支持的版本可以继续启动；只有版本低于最低支持版本时才会要求先完成更新。更新无需用户操作。
...
确认从 `Chimera++` 入口启动，而不是原版 Codex。也可在管理工具的「诊断」「日志」页查看注入状态。
...
若接口正常但插件仍超时，通常是 CDP bridge 或脚本缓存问题：重启 Chimera++，或查看日志中的 `renderer.script_loaded` / `bridge.request`。
...
Chimera++ 作为修改后的整体以 [AGPL-3.0-only](LICENSE) 发行。第三方许可、上游基线与许可证变更时间线见 [NOTICE](NOTICE)。对应源代码发布在公开仓库：<https://github.com/Duojiyi/chimera-codex>。
...
Chimera++ 是外部增强工具，不修改 Codex App 原始文件。Codex App 更新后若页面结构变化，可能需要更新注入脚本。
