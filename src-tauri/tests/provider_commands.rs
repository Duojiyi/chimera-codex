...
fn settings_path(home: &Path) -> PathBuf {
    home.join(chimera_plus_plus_lib::product_policy::PRODUCT_DATA_DIR)
        .join("settings.json")
}
...
fn switch_provider_updates_claude_live_and_state() {
    ...
    let settings_path = chimera_plus_plus_lib::get_claude_settings_path();
    ...
    assert!(
        db_path.exists(),
        "switching provider should persist to Chimera++ database"
    );
