...
fn sync_codex_provider_writes_config_without_touching_auth() {
    ...
    let auth_path = chimera_plus_plus_lib::get_codex_auth_path();
    let config_path = chimera_plus_plus_lib::get_codex_config_path();
...
requires_openai_auth = true
"#;
    chimera_plus_plus_lib::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
"#
    ...
    let toml_text = fs::read_to_string(chimera_plus_plus_lib::get_codex_config_path())
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = chimera_plus_plus_lib::get_codex_config_path();
    ...
    chimera_plus_plus_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = chimera_plus_plus_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        chimera_plus_plus_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        chimera_plus_plus_lib::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_single_server_to_codex_fails_closed_on_invalid_toml() {
    ...
    let err = chimera_plus_plus_lib::sync_single_server_to_codex(
        &config,
        "srv",
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    match err {
        chimera_plus_plus_lib::AppError::Config(msg) => {
    ...
    // 确认未产生任何 live 配置文件
    assert!(
        !chimera_plus_plus_lib::get_codex_auth_path().exists(),
        "auth.json should not be created on failure"
    );
    assert!(
        !chimera_plus_plus_lib::get_codex_config_path().exists(),
...
"#;
    ...
    chimera_plus_plus_lib::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        chimera_plus_plus_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = chimera_plus_plus_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        chimera_plus_plus_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        chimera_plus_plus_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = chimera_plus_plus_lib::import_from_codex(&mut config).expect("import codex");
...
"#,
    ...
    config.mcp.servers.as_mut().unwrap().insert(
        "existing".to_string(),
        chimera_plus_plus_lib::McpServer {
            ...
            apps: chimera_plus_plus_lib::McpApps {
                claude: false,
                codex: false, // 初始未启用
                gemini: false,
                grokbuild: false,
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    chimera_plus_plus_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = chimera_plus_plus_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed =
        chimera_plus_plus_lib::import_from_claude(&mut config).expect("import from claude");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(chimera_plus_plus_lib::product_policy::PRODUCT_DATA_DIR);
