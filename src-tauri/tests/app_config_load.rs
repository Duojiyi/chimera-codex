...
use chimera_plus_plus_lib::{AppError, MultiAppConfig};
...
fn load_valid_v2_config_succeeds() {
    ...
    assert!(loaded
        .get_manager(&chimera_plus_plus_lib::AppType::Claude)
        .is_some());
    assert!(loaded.get_manager(&chimera_plus_plus_lib::AppType::Codex).is_some());
}
