//! Panic Hook 模块
//!
//! 在应用崩溃时捕获 panic 信息并记录到 Chimera++ 配置目录的 `crash.log`。
//! 便于用户和开发者诊断闪退问题。
...
fn default_app_config_dir() -> PathBuf {
    dirs::home_dir()
        ...
        .join(crate::product_policy::PRODUCT_DATA_DIR)
}
...
"#
        ...
        if saved {
            eprintln!("\n[Chimera++] Crash log saved to: {}", log_path.display());
        }
...
mod tests {
    ...
    fn test_crash_log_path() {
        ...
        assert!(path.to_string_lossy().contains(".chimera-plus-plus"));
    }
