...
//!
//! Parses Chimera++ and legacy CC Switch URLs into import requests.
...
/// Parse a `chimera://` or legacy `ccswitch://` URL into an import request.
///
/// Expected format:
...
pub fn parse_deeplink_url(url_str: &str) -> Result<DeepLinkImportRequest, AppError> {
    ...
    if scheme != crate::product_policy::PRODUCT_DEEP_LINK_SCHEME
        && scheme != crate::product_policy::LEGACY_DEEP_LINK_SCHEME
    {
        return Err(AppError::InvalidInput(format!(
            "Invalid Chimera++ deep-link scheme: '{scheme}'"
        )));
    }
