...
> **实现状态（v2.3.0）**：功能已落地并接入 Chimera 的「会话」标签页。与本文档的原始范围有两处差异：来源不止 Codex / Claude Code，实际还包含 Gemini CLI、Grok Build、OpenCode；「在终端恢复」仍仅 macOS 可用，其他平台退化为复制恢复命令，而不是整个功能不可用。本文档保留原始需求记录，不再随实现更新。
