...
use chimera_theme::fingerprint::{
    ExpectedFingerprint, FingerprintError, KillSwitchError, KillSwitchTrustAnchor,
    PROBE_SCHEMA_VERSION, ProbeInput, SignedKillSwitch, SkinFuse, TripReason, compute_fingerprint,
};
...
fn a_digest_mismatch_is_refused() {
    let candidate = compute_fingerprint(&probe("2.0.0", &[".title"]));
    let expected =
        ExpectedFingerprint::parse(valid_expected_json(VALID_DIGEST).as_bytes()).unwrap();
...
fn a_kill_switch_that_explicitly_says_do_not_disable_leaves_the_fuse_engaged() {
    ...
    assert!(
        fuse.skin_enabled(),
        "disable_skin:false must not trip anything"
    );
}
